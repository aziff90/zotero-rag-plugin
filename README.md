...
2. **Chunking & Vector Search:** Dynamically splits documents into ~1000 character overlapping chunks on the fly. It utilizes a robust `Transformers.js` model running purely inside Zotero to search the text vectors.
3. **Chat Interface:** Features an integrated React app loaded directly in a Zotero Dialog window to process chat history and stream from Gemini 1.5 Flash.
...
You can customize the RAG Assistant globally by configuring the LLM Backend:
...
5. Wait for the vectors to initialize, and start asking questions about your sources!
